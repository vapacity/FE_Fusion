...
def recall_at_n_with_distance(query_loader, database_features, query_gps_data, database_gps_data, model, timestamps_list, N=5, distance_threshold=75, use_dift=False):
    ...
    correct_count = 0
    query_global_idx = 0
    database_features_copy = database_features.clone().detach().to("cuda")
    ...
    with torch.no_grad():
        ...
        for test_batch_idx, (frames, event_volumes, timestamp_batch) in tqdm(enumerate(query_loader), desc="Calculating Cosine Similarity"):
            ...
            similarity_scores = cos(query_features.unsqueeze(1), database_features_copy.unsqueeze(0))    #[B, length]
