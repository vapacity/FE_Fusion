...
if args.use_frame:
    model = FE_Net.MainNet(channel_sizes, use_event=False).cuda()  # Only frames
elif args.use_event:
    model = FE_Net.MainNet(channel_sizes, use_frame=False).cuda()  # Only events
else:
    model = FE_Net.MainNet(channel_sizes).cuda()  # Both frames and events
